...
- `AGENTS.md`：面向 AI 代理、维护者与贡献者，着重于开发说明、协作规则、验证流程与文档约束。
...
- `App.tsx`：应用根组件与页面编排。
- `store/`：全局状态与交互入口。
- `components/`：Sidebar、项目视图、详情抽屉、设置面板、同步相关 UI 等。
...
1. 先判断改动主要落在哪一层：`main`、`preload`、`renderer` 还是 `shared`。
2. 在最合适的一层实现改动，避免跨层耦合扩散。
3. 为功能改动补充或更新测试。
4. 运行类型检查、构建与测试。
...
- 单元测试与源码同级放置。
- 涉及Electron 主进程 / preload / IPC / worker / 文件系统监听 / 真实同步链路等功能时，不能只依赖浏览器 mock，应该按照如下流程验证：
...
如果改动涉及以下能力，默认流程是在类型检查、构建和相关测试通过后，再启动真实应用手动验证：
