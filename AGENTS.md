...
- 不使用不必要的圆角外框和分割线，优先使用间距划分，其次可使用背景色区分层级。
- 不添加“显而易见”的说明性 note 文本；若界面结构本身已能表达含义，则不要再重复解释。
- 去掉不必要的包裹层次，避免为列表、区块、表单机械套用卡片；优先保持结构直接、信息密度高、视觉极简。
...
- 涉及 **Electron 主进程 / preload / IPC / worker / 文件系统监听 / 真实同步链路** 的功能时，**不能只依赖浏览器 mock** 判断是否正确。
- 这类功能的默认验证流程：
