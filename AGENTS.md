...
- `local` 关注“这个项目在这台机器上在哪里”：扫描根、本地路径 binding、warning、ignoredPaths、同步状态等；历史 `ui` 字段仅作兼容保留，运行时主题 / 视图以应用级偏好为准。
- 项目目录的文件系统修改时间属于运行时数据，不应写入 `local`；动态标签等需要目录 `mtime` 的功能应在运行时读取并可做短时缓存。
