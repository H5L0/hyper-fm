...
- **动作快捷入口**：支持配置全局动作和项目级动作；项目级动作可选择保存在本地或共享配置中，快速打开 IDE、终端或执行自定义脚本。
...
- **主区域**：浏览项目卡片、搜索项目。
- **项目详情**：点击项目卡片查看详细信息，可在"信息 / 同步 / 动作"标签页中编辑项目数据和项目级动作；动作页修改会先保存在草稿中，点击保存后生效。
- **文件视图**：列出项目所有文件，用于打开文件、收藏文件、检查忽略文件。
- **设置面板**：管理配置文件、扫描根、忽略规则、全局动作和同步设置。
...
3. 在主界面搜索、筛选和浏览项目。
4. 点击项目进入详情页，编辑项目信息、同步规则和项目级动作；若修改了动作，需要点击面板保存后才会落盘。

#### 使用动作
1. 在【软件设置】中配置适用于所有项目的全局动作。
2. 在项目详情的【动作】标签页中配置项目级动作，并按需要选择保存到"本地"或"共享"。
3. 在项目详情右上角动作菜单或托盘项目菜单中直接运行动作。
